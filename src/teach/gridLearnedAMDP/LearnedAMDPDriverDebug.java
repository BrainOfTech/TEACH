...
public class LearnedAMDPDriverDebug {
	...
	// Learning: Prop Functions for In-Room, Expected Reward (predictive
	// function?) for Transition, abstract action policies...
	...
	public static void main(String[] args) {
		...
		// PropositionalFunction pfL0 = new
		// PF_InCoordinateSpace(PF_AGENT_IN_COORDINATE_SPACE, new
		// String[]{CLASS_COORDINATE_SPACE});
		...
		// Note: State is object-oriented, so Prop Functions can pull out
		// objects from state
		...
		// TODO: Set initial state and Derive L1 state from AMDPStateMapping
		// (class defined elsewhere)

		// TODO: Task Nodes? - Probably not applicable in learned case

		// TODO: Policy Generators - One for each level. (This is where the
		// planning gets swapped.)
	...
	public static Data executeAmdp(Boolean experiment, int[] sl, String gl) {
		...
		Trajectory[] traj_array = TestsDriver.generateTrajectories(file_directory);// TODO:
																					// Refactor
																					// Loading
		...
		// PF_InCoordinateSpace(PF_AGENT_IN_COORDINATE_SPACE, new
		// String[]{CLASS_COORDINATE_SPACE});
		// GroundedProp gpL0 = new GroundedProp(pfL0,new
		// String[]{goal_location}); //Ground generic proposition to goal
		//
		// GroundedPropSC L0sc = new GroundedPropSC(gpL0);
		// GoalBasedRF L0rf = new GoalBasedRF(L0sc, 1, -1);
		// GoalConditionTF L0tf = new GoalConditionTF(L0sc);
		...
		gw.setMapToFourRooms(); // four rooms layout, 11x11 grid world
								// (essential)
		OOSADomain domainL0 = gw.generateDomain(); // generate the grid world
													// domain
		...
		// Create States
		// L0: room object (room assignment numbered top-left proceeding
		// counterclockwise)
		...
		// L0 State-->Starting location(GridAgent), Rooms(AmdpL0Room), Ending
		// Location(GridLocation)
		AmdpL0State L0_state = new AmdpL0State(new AmdpL0Agent(start_location[0], start_location[1], agent_name),
				L0_rooms);
		...
		ValueFunctionVisualizerGUI gui = GridWorldVisualizer.getGridWorldValueFunctionVisualization(allStates, 11, 11,
				(ValueFunction) planner, p);
		gui.initGUI();
		...
		//
		// ActionType aget_to_room =
		// domainL1.getAction(AmdpL1Domain.ACTION_AGENT_TO_ROOM);
		//
		...
		//
		// TaskNode a2rt = new LearnedL1TaskNode(aget_to_room,
		// gw.generateDomain(), L1Subtasks, lsm);
		// TaskNode L1Root = new RootTaskNode("root",new
		// TaskNode[]{a2rt},domainL1, L1tf,L1rf);//TODO:check for looping error?
		//
		// List<AMDPPolicyGenerator> pgList = new
		// ArrayList<AMDPPolicyGenerator>();
		// pgList.add(0,new l0PolicyGenerator(domainL0));
		// pgList.add(1,new l1PolicyGenerator(domainL1, lsm));
		//
		// AMDPAgent agent = new
		// AMDPAgent(L1Root.getApplicableGroundedTasks(L1_state).get(0),pgList);
		// SimulatedEnvironment env = new
		// SimulatedEnvironment(gw.generateDomain(), L0_state);
		//
		...
		// goal_location, L0_state);
		// new EpisodeSequenceVisualizer(v, domainL0, Arrays.asList(e));
		//
		// HashableStateFactory hashingFactory = new
		// SimpleHashableStateFactory();
		// Planner planner = new ValueIteration(domainL0, 0.99, hashingFactory,
		// 0.001, 100);
		// Policy p = planner.planFromState(L0_state);
		// List<State> allStates =
		// StateReachability.getReachableStates(L0_state, domainL0,
		// hashingFactory);
		// ValueFunctionVisualizerGUI gui =
		// GridWorldVisualizer.getGridWorldValueFunctionVisualization(
		// allStates, 11, 11, (ValueFunction)planner, p);
		// gui.initGUI();
		//
		//// Planner planner2 = new ValueIteration(domainL1, 0.99,
		// hashingFactory, 0.001, 100);
		//// Policy p2 = planner.planFromState(L1_state);
		//// List<State> allStates2 =
		// StateReachability.getReachableStates(L1_state, domainL1,
		// hashingFactory);
		//// ValueFunctionVisualizerGUI gui2 =
		// GridWorldVisualizer.getGridWorldValueFunctionVisualization(
	...
	public static class l0PolicyGenerator implements AMDPPolicyGenerator {
		...
		public Policy generatePolicy(State s, GroundedTask gt) {
			...
			l0.setModel(new FactoredModel(((FactoredModel) l0.getModel()).getStateModel(), gt.rewardFunction(),
					gt.terminalFunction()));
	...
	public static class l1PolicyGenerator implements AMDPPolicyGenerator {
		...
		public Policy generatePolicy(State s, GroundedTask gt) {
			...
			l1.setModel(new FactoredModel(((FactoredModel) l1.getModel()).getStateModel(), gt.rewardFunction(),
					gt.terminalFunction()));
